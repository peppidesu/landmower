...
export default function AddLinkForm() {
  ...
  const [validateRes, setValidateRes] =
    useState<ValidateAddLinkResponse | null>(null);
  ...
  return (
    <div class="max-w-2xl flex flex-col items-center px-4 gap-5 w-full">
      ...
      >
        <input
          class={`w-full ${linkTouched && invalid?.link ? "invalid" : ""}`}
          name="link"
          type="text"
        ...
        <div class="flex items-center gap-2 pt-2 w-full flex-wrap">
          <label
            for="customize"
            class="grid grid-cols-[1em_auto] mx-2 gap-4 items-center text-gray-300"
          >
            <input
              type="checkbox"
              id="customize"
          ...
          <input
            class={`w-2xs grow shrink-0 ${keyTouched && invalid?.key ? "invalid" : ""}`}
            name="key"
            type="text"
      ...
      {addLinkRes && addLinkRes.status === "success" && (
        <CopyButton
          text={`${import.meta.env.VITE_SERVER_URL}${addLinkRes.data.key}`}
        />
      )}
    </div>
  );

  ...
  return (
    <div class="flex flex-col p-3 bg-gray-800/50 rounded-md">
      <div class="flex items-center text-lg">
        ...
        <button
          onClick={onDelete}
          class="ml-auto grid place-items-center aspect-square h-full delete-button"
        >
          <i class="ti ti-trash col-span-full row-span-full"></i>
        </button>
      </div>
      <div class="overflow-hidden w-full">{entry.link}</div>
      <div class="text-gray-400 text-sm ml-5">
        Used {entry.metadata.used} times
      </div>
  ...
  return (
    <div class={`dialog-wrapper ${visible ? "" : "hidden"}`}>
      <div class="bg-gray-800 mx-4 p-5 rounded-md max-w-lg">
        ...
        <div class="text-gray-300 mt-2 text-center break-all">
          {import.meta.env.VITE_SERVER_URL}
          {entry?.key ?? ""}
        </div>
        <div class="flex gap-5 mt-4">
          <button class="pill w-full btn-danger" onClick={onDelete}>
            Delete
          </button>
          <button class="pill w-full btn-primary" onClick={onCancel}>
            Cancel
          </button>
        </div>
      </div>
